...
export default function Signup() {
  ...
  const handleClick = async (e) => {
    ...
    setAddress(location);
    setCredentials({ ...credentials, geolocation: location }); // Setting geolocation
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await fetch("http://localhost:5000/api/auth/createuser", {
      ...
      body: JSON.stringify({
        ...
        location: address,  // Ensure location (address) is passed correctly
      }),
    });
  
    const json = await response.json();
    console.log(json);  // Log full response
  
    if (json.success) {
      localStorage.setItem('token', json.authToken);
      navigate("/login");
    } else {
      console.log(json.errors);  // Log errors from the backend
      alert(json.errors[0].msg); // Display the first error
    }
  };
